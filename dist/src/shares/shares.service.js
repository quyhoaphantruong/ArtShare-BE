...
const share_details_dto_1 = require("./dto/response/share-details.dto");
let SharesService = class SharesService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async createShare(dto, userId) {
        ...
        return (0, class_transformer_1.plainToClass)(share_details_dto_1.ShareDetailsDto, share);
    }
    ...
    async verifyShareAlreadyExists(dto, userId) {
        const existing = await this.findShare(dto.target_id, dto.target_type, userId);
        if (existing)
            throw new common_1.BadRequestException('You have already shared this');
    }
    async findShare(targetId, targetType, userId) {
        if (targetType === client_1.TargetType.POST) {
