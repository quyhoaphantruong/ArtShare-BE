...
import { ViewTab } from './dto/view-report.dto';
  ...
  export class ReportService {
      ...
      try {
        switch (targetType) {
          case ReportTargetType.POST:
            if (isNaN(targetId)) throw new BadRequestException(`Invalid Post ID format: ${targetId}`);
          ...
          case ReportTargetType.BLOG:
             if (isNaN(targetId)) throw new BadRequestException(`Invalid Blog ID format: ${targetId}`);
          ...
          case ReportTargetType.COMMENT:
             if (isNaN(targetId)) throw new BadRequestException(`Invalid Comment ID format: ${targetId}`);
    ...
    async createReport(
      ...
      if (existingReport) {
        throw new ConflictException('You have already submitted a report for this item.');
      }
    ...
    async findPendingReports(options: { skip?: number; take?: number }): Promise<Report[]> {
